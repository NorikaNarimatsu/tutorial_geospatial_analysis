...
```{r student }
# map(database = "state", regions = "__________")
```

```{r student2 }
# ____.cities <- subset(us.cities, subset = (country.etc == "_____"))
# sind <- order(___.cities$pop, decreasing = TRUE) # order by population
# top7 <- sind[1:7] # seven cities with largest population

# map(database = "state", regions = "________")

# points(x = ____.cities$long[top7], y = ____.cities$lat[top7], pch = 19)

# text(x = ____.cities$long[top7] , y = ____.cities$lat[top7],
  #    label = ____.cities$name[top7])
# title("Largest cities of ______")
...
# for (i in 1:nrow(___)) {
#    if ((_____$state[i] != "HI") & (_____$state[i] != "AK") &
#        (_____$state[i] != "DC")) {
#        maps::map(database = "state", regions = _____$state.name[i],
#            col = ifelse(_____________ , "red", "blue"),
#            fill = TRUE, add = TRUE)
#    }
#}
#title("___________") # add a title
```
...
#     if ((___$state[i] != "HI") & (___$state[i] != "AK") &
#         (___$state[i] != "DC")) {
#         map(database = "state", regions = _________, 
#             col = rgb(red = _______, blue = ________,
#                 green = 0), fill = TRUE, add = TRUE)
#     }
# }
# title("_______") # add a title
```
### Questions 2:
...
### Questions 3:
Do you have any remaining questions about this pcw, especailly coding part? Write down your questions here and bring your confusions to the class! 
EXTRA CELL
